...
def checkAnswer(viber_id, text):
    ...
    if text[0] == user.questionCount_session:
        if text[1] == (session.query(Word).join(User).filter(Word.id == user.currentword_id)).first().translation:
            ...
            session.commit()
            viber.send_messages(viber_id, [TextMessage(text=f"Вопрос №{user.questionCount_session + 1}. Верно!")])
        else:
            viber.send_messages(viber_id, [TextMessage(text=f"Вопрос №{user.questionCount_session + 1}. Неверно!")])
...
def checkEndSession(viber_id):
    ...
    if user.questionCount_session >= set.session_words:
        final = TextMessage(text=f"Количество правильных ответов: {user.correct_answers_session} из {set.session_words}")
